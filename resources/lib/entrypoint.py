...
def directory_item(label, path, folder=True):
    ...
    xbmcplugin.addDirectoryItem(
        handle=int(sys.argv[1]), url=path, listitem=listitem, isFolder=folder
    )
...
def show_main_menu(content_type=None):
    ...
    for i in range(totalnodes):
        ...
        if node_type == v.CONTENT_TYPE_PHOTO and content_type == "image":
            ...
            node_type
            in (v.CONTENT_TYPE_ARTIST, v.CONTENT_TYPE_ALBUM, v.CONTENT_TYPE_SONG)
            and content_type == "audio"
        ):
            directory_item(label, path)
        elif (
            node_type
            in (v.CONTENT_TYPE_MOVIE, v.CONTENT_TYPE_SHOW, v.CONTENT_TYPE_MUSICVIDEO)
    ...
    # Plex user switch
    directory_item(
        "%s%s" % (utils.lang(39200), utils.settings("username")),
    ...
    directory_item(utils.lang(39204), "plugin://%s?mode=manualsync" % v.ADDON_ID)
...
def show_section(section_index):
    ...
    plex_type = (
        v.PLEX_TYPE_MOVIE if content == v.CONTENT_TYPE_MOVIE else v.PLEX_TYPE_SHOW
    )
...
def show_listing(xml, plex_type=None, section_id=None, synched=True, key=None):
    ...
    try:
        ...
        LOG.info("xml received from the PMS is empty: %s, %s", xml.tag, xml.attrib)
        return
    api = API(xml[0])
    ...
    LOG.debug(
        "show_listing: section_id %s, synched %s, key %s, plex_type %s, "
        "content type %s",
        section_id,
        synched,
        key,
    ...
    all_apis = mass_api(xml, check_by_guid=key == "watchlist")

    if key == "watchlist":
        # filter out items in the wrong section id when it's specified
        if section_id is not None:
            all_apis = [
                api for api in all_apis if api.section_id == utils.cast(int, section_id)
            ]
    ...
    for api in all_apis:
        ...
        if item is None:
            continue
        # For non-DB watchlist items, inject ratingKey so the context menu
        # remove-from-watchlist action works for discover/non-library items
        if key == "watchlist" and not api.kodi_id:
            rk = api.xml.get("ratingKey")
            if rk:
                item.setdefault("extraproperties", {})["ratingKey"] = rk
...
def get_video_files(plex_id, params):
    ...
    for root, dirs, files in path_ops.walk(path):
        for directory in dirs:
            ...
            xbmcplugin.addDirectoryItem(
                handle=int(sys.argv[1]), url=item_path, listitem=listitem, isFolder=True
            )
        for file in files:
            item_path = path_ops.path.join(root, file)
            listitem = ListItem(item_path, path=item_path)
            xbmcplugin.addDirectoryItem(
                handle=int(sys.argv[1]), url=file, listitem=listitem
            )
        break
...
def extra_fanart(plex_id, plex_path):
    ...
    """
    LOG.debug("extra_fanart alled with plex_id: %s, plex_path: %s", plex_id, plex_path)
    if not plex_id:
    ...
    fanart_dir = path_ops.translate_path("special://thumbnails/plex/%s/" % plex_id)
    _wait_for_auth()
    if not path_ops.exists(fanart_dir):
        ...
        for count, backdrop in enumerate(backdrops):
            ...
            xbmcplugin.addDirectoryItem(
                handle=int(sys.argv[1]), url=art_file, listitem=listitem
            )
...
def browse_plex(
    ...
    """
    LOG.debug(
        "Browsing to key %s, section %s, plex_type: %s, synched: %s, "
        'prompt "%s", args %s',
        key,
        section_id,
        plex_type,
        ...
        for hub in xml:
            ...
            for entry in hub:
                ...
                if api.plex_type == v.PLEX_TYPE_TAG:
                    ...
                    entry.attrib["tag"] = "%s: %s" % (hub.get("title"), api.tag_label())
                answ.append(entry)
        xml = answ
